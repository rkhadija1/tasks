...
export const removeDollars = (amounts: string[]): number[] => {
    const converted = amounts.map((amount) => {
        const removedAmount = amount.replace(/^\$/, "");
        const parsed = parseInt(removedAmount);
        return isNaN(parsed) ? 0 : parsed;
    });
    return converted;
};
...
export const shoutIfExclaiming = (messages: string[]): string[] => {
    const deleted = messages.filter(
        (message): boolean => !message.includes("?")
    );

    const modifiedMessage = deleted.map((message) =>
...
export function countShortWords(words: string[]): number {
    const shortWords = words.filter(
        (smallWord: string): boolean => smallWord.length < 4
    );
...
export function makeMath(addends: number[]): string {
    ...
    const finalString = addends.join("+");
    if (sum === 0) {
        return sum + "=" + 0;
    }
