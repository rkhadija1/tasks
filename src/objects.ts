...
export function toShortForm(question: Question): string {
    // you can also use substring(0, 10) or [...question.name].slice(0, 10).join(''), or question.name.match(/^.{0,10}/)[0]

    const combinedRepresentation =
        question.id + ": " + question.name.slice(0, 10);
...
export function toMarkdown(question: Question): string {
    let formattedRepresentation = "# " + question.name + "\n" + question.body;
    // if (question.body) {
    //     formattedRepresentation += question.body + "\n";
    // }
    if (question.type === "short_answer_question") {
    ...
    if (question.type === "multiple_choice_question") {
        ...
        formattedRepresentation += optionsList.join("");
    }
    return formattedRepresentation;
}
...
export function publishQuestion(question: Question): Question {
    const invertedQuestion: Question = { ...question };
    invertedQuestion.published = !question.published;
...
export function mergeQuestion(
    ...
    const newQuestionMade = { ...contentQuestion };
    newQuestionMade.id = id;
    newQuestionMade.name = name;
    newQuestionMade.points = points;
    newQuestionMade.published = false;
    return newQuestionMade;
}
